...
def calculate_indicators(df):
    ...
    slowk, slowd = talib.STOCH(df['high'], df['low'], df['close'],
                               fastk_period=STOCHASTIC_SETTINGS['k_period'],
                               slowk_period=STOCHASTIC_SETTINGS['d_period'],
                               slowd_period=STOCHASTIC_SETTINGS['d_period'])
    df['%K'] = slowk
    df['%D'] = slowd
    
    # Calculate Bollinger Bands
    upper, middle, lower = talib.BBANDS(df['close'],
                                        timeperiod=BOLLINGER_SETTINGS['window'],
                                        nbdevup=BOLLINGER_SETTINGS['num_std'],
                                        nbdevdn=BOLLINGER_SETTINGS['num_std'])
    df['BB_upper'] = upper
    df['BB_middle'] = middle
    df['BB_lower'] = lower
...
def run_rsi_strategy(data_path=CSV_FILE_PATH, initial_balance=INITIAL_BALANCE, 
                     stop_loss_pct=STOP_LOSS_PERCENT, target_profit_pct=TARGET_PROFIT_PERCENT, 
                     risk_per_trade=RISK_PER_TRADE, transaction_cost_pct=TRANSACTION_COST_PERCENT):
    ...
    entry_price = None
    trade_history = []
    cooling_period = COOLDOWN_PERIODS
    ...
    for i in range(50, len(df)):
        ...
        # Position exit logic
            ...
            if current_price <= stop_loss or current_price >= target or bearish_signals.iloc[i]:
                profit -= profit * transaction_cost_pct / 100
                balance += profit
                ...
                log_trade("LONG", "EXIT", current_price, current_time, 
                          reason=trade_history[-1]['reason'], 
            ...
            if current_price >= stop_loss or current_price <= target or bullish_signals.iloc[i]:
                ...
                log_trade("SHORT", "EXIT", current_price, current_time, 
                          reason=trade_history[-1]['reason'], 
                          profit=profit, balance=balance)
                position = None
